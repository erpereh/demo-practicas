...
import { Search, Plus, Trash2, X, Building2, Loader2, AlertCircle } from "lucide-react";
...
export default function ClientesPage() {
  ...
  // Form (ahora acorde a la tabla real)
  ...
  const cargarClientes = async () => {
    try {
      ...
      setErrores({ general: e?.message || "Error cargando clientes" });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGuardar = async () => {
    if (!idCliente.trim() || !nombre.trim() || !cif.trim()) {
      setErrores({ general: "Sociedad, ID Cliente, Nombre y CIF son obligatorios" });
      return;
    }
  ...
  // 3) Archivar (PATCH -> delete)
  const handleArchivar = async (id_cliente: string) => {
    if (!confirm(`¿Archivar (eliminar) el cliente ${id_cliente}?`)) return;

    try {
      const res = await fetch(`${API_URL}/api/clientes/${encodeURIComponent(id_cliente)}/archivar`, {
        method: "PATCH",
      });
      if (!res.ok) {
        const msg = await parseFastApiError(res);
              ...
              clientesFiltrados.map((c) => (
                <tr key={c.id_cliente} className="hover:bg-gray-50">
                  <td className="px-6 py-4">
                    ...
                    <p className="text-xs text-gray-500">ID: {c.id_cliente} · CIF: {c.cif}</p>
                  </td>
                  <td className="px-6 py-4 text-gray-600">{c.persona_contacto || "-"}</td>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6">
            <div className="flex justify-between items-center mb-6 border-b pb-4">
              <h3 className="text-lg font-bold text-quality-dark flex gap-2 items-center">
                <Building2 className="text-quality-red" /> Nuevo Cliente
              </h3>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="col-span-2">
                  <label className="text-sm font-medium block mb-1">ID Cliente</label>
                  <input className="w-full border rounded-lg px-3 py-2" placeholder="ATOS, CYC..." value={idCliente} onChange={(e) => setIdCliente(e.target.value)} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium block mb-1">CIF/NIF</label>
